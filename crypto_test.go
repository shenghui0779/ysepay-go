...
func TestDesEcbCrypto(t *testing.T) {
	...
	cipher, err := DESEncryptECB(key, []byte(data))
	assert.Nil(t, err)
	assert.Equal(t, "B/id4J/r9rstbbNi3mONwA==", base64.StdEncoding.EncodeToString(cipher))

	plain, err := DESDecryptECB(key, cipher)
	assert.Nil(t, err)
